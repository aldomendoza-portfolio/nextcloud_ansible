...
## Mailcow (Servidor de Correos)

Este repositorio incluye opcionalmente el despliegue de **Mailcow**, una suite completa de correo electrónico. Mailcow funciona detrás de Traefik para no entrar en conflicto con Nextcloud.

1. Asegúrate de configurar los siguientes registros DNS apuntando a la IP de tu servidor (`<TU_IP_PUBLICA>`):
   - `mailcow.tu_dominio.com` (Tipo A)
   - `autodiscover.tu_dominio.com` (Tipo CNAME apuntando a `mailcow...`)
   - `autoconfig.tu_dominio.com` (Tipo CNAME apuntando a `mailcow...`)
   - Además de los registros habituales para correo (MX, SPF, DKIM, DMARC).

2. Accede al panel de administración en `https://mailcow.tu_dominio.com` e inicia sesión con las credenciales por defecto (`admin` / `moohoo`). ¡Asegúrate de cambiar esta contraseña inmediatamente!
